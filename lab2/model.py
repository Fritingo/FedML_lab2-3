...
class LSTM_shakespeare_1L(torch.nn.Module):
    def __init__(self, n_vocab=80, embedding_dim=64, hidden_dim_1=256, nb_layers_1=1):
        ...
        # self.hidden_dim_2 = torch.nn.Linear(hidden_dim_1, 128)
        # self.hidden_dim_3 = torch.nn.Linear(self.hidden_dim_2, 256)
        # self.hidden1out = torch.nn.Linear(self.hidden_dim_3, 128)
        self.drop1 = nn.Dropout(p=0.5)
        self.hidden2out = torch.nn.Linear(128, n_vocab)
        self.hidden3out = torch.nn.Linear(n_vocab,n_vocab)
    ...
    def forward(self, seq_in, state=None):
        ...
        ht = lstm_out[-1]
        out = torch.nn.functional.relu6(self.hidden1out(ht))

        out = self.hidden2out(self.drop1(out))
        
        out = self.hidden3out(self.drop1(out))
